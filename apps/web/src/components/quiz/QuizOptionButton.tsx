  ...
  if (imageUrl) {
    return (
      <button
        onClick={onClick}
        disabled={isDisabled}
        className={`group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border text-left transition-all duration-200 ${
          isSelected
            ? "border-cyan-300/70 bg-cyan-300/10 shadow-2xl shadow-cyan-500/20"
            : "border-white/10 bg-white/[0.06] shadow-xl shadow-black/20 hover:-translate-y-0.5 hover:border-cyan-300/40 hover:bg-white/[0.09] hover:shadow-2xl hover:shadow-cyan-500/10"
        } ${isDisabled ? "cursor-not-allowed opacity-60 hover:translate-y-0" : ""}`}
        style={{
          backdropFilter: "blur(14px)",
        }}
      >
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-gradient-to-br from-brand-500/10 to-accent-500/10">
          <img
            src={imageUrl}
            alt={label}
          ...
          <div
            className={`absolute left-3 top-3 flex h-11 w-11 items-center justify-center rounded-2xl text-sm font-black transition-colors duration-200 ${
              isSelected
                ? "bg-cyan-300 text-slate-950 shadow-lg shadow-cyan-400/40"
